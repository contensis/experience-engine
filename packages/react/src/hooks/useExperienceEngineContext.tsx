...
import { ExperienceEngineContext } from "@contensis/experience-engine";
import {
  IExperienceEngineReactContext,
  ExperienceEngineReactContext,
} from "../context/ExperienceEngineReactContext";

/** React hook that returns experience engine state for you to provide personalized experiences in your components */
export const useExperienceEngineContext = () => {
  const context = useContext(ExperienceEngineReactContext);

  const [state, setState] = useState<IExperienceEngineReactContext>({
    audiences: [],
    signals: [],
    context: context as ExperienceEngineContext,
    computed: [],
    isAudience: () => false,
    matched: [],
    pageViews: {
    ...
    percentile: 0,
    setAttributes: () => {},
    getAttributes: (() => null) as ExperienceEngineContext["getAttributes"],
    overrideAttributes: () => {},
    toggleAudience: () => {},
    t: 0,
  });
  ...
  if (!context) {
    throw new Error(
      "useExperienceEngineContext must be used within a <ExperienceEngineProvider> provider"
    );
  }
  return state;
};
